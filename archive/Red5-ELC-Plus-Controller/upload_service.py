    ...
    with zipfile.ZipFile(zip_path, 'r') as zf:
        for entry in zf.namelist():
            ...
            if base_name == 'app.py':
                ...
                continue
            if base_name == 'collector.py':
                skipped.append({'file': clean_name, 'reason': 'Collector (collector.py) not auto-deployed - register via enteliWEB'})
                continue
            if ext.lower() == '.py':
                # Preserve elc/ package tree under PLUGINS_ROOT.
                if clean_name.startswith('pgpy/elc/'):
                    target_root = PLUGINS_ROOT
                    clean_name = clean_name[len('pgpy/'):]
                elif clean_name.startswith('elc/'):
                    target_root = PLUGINS_ROOT
                else:
                    target_root = PLUGINS_ROOT
