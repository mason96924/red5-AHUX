...
def _start_background_band_generator():
    ...
    try:
        ...
        if os.path.exists(CONFIG_PATH):
            band_csv_generator.generate_all(CONFIG_PATH, DATA_ROOT)
            band_csv_generator.start_background(CONFIG_PATH, DATA_ROOT, interval=300)
...
def regenerate_band_csv():
    ...
    try:
        ...
        files = band_csv_generator.generate_all(cfg, DATA_ROOT)
...
def get_band_guide():
    ...
    filepath = os.path.join(DATA_ROOT, 'band_guide.csv')
    if not os.path.exists(filepath):
        return jsonify({'error': 'band_guide.csv not found — trigger /api/band-csv/regenerate first'}), 404
    return send_from_directory(DATA_ROOT, 'band_guide.csv', mimetype='text/csv')
...
def get_band_csv(ahu_id):
    ...
    return send_from_directory(DATA_ROOT, filename, mimetype='text/csv')
