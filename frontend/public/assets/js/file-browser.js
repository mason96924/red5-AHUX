...
const FileBrowserModal = ({
    isOpen, onClose, data, loading, currentPath,
    onNavigate, onRefresh, onUpload, onCreateDir, onInitScaffold,
        ...
        >
            ...
            >
                ...
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <button onClick={onRefresh} className="px-2 py-1 bg-slate-800 border border-slate-600 text-slate-400 rounded text-[8px] font-black uppercase tracking-wider hover:border-sky-500 hover:text-sky-400 transition-all">Refresh</button>
                    <button onClick={onUpload} className="px-2 py-1 bg-slate-800 border border-emerald-600 text-emerald-400 rounded text-[8px] font-black uppercase tracking-wider hover:bg-emerald-900/30 transition-all">Upload File</button>
                    <button onClick={onCreateDir} className="px-2 py-1 bg-slate-800 border border-amber-600 text-amber-400 rounded text-[8px] font-black uppercase tracking-wider hover:bg-amber-900/30 transition-all">New Folder</button>
                    <button onClick={onInitScaffold} className="px-2 py-1 bg-slate-800 border border-purple-600 text-purple-400 rounded text-[8px] font-black uppercase tracking-wider hover:bg-purple-900/30 transition-all">Init Scaffold</button>
                </div>
                
                {/* Breadcrumb navigation */}
                <div className="flex items-center gap-1 mb-3 text-[10px] font-mono">
                    <button onClick={() => onNavigate('')} className={`px-1.5 py-0.5 rounded hover:bg-slate-700 transition-colors ${!currentPath ? 'text-sky-400 font-black' : 'text-slate-400 hover:text-white'}`}>/root/data</button>
