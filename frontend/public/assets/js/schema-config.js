...
const ALIGNER_TYPES = [
    'centrifugal_fan', 'rectangular_fan', 'rectangular_fan_aligner', 'hydration_valve', 'hydration_valve_aligner', 'damper', 
    'neon_pipe_coil', 'diff_pressure_switch', 'differential_pressure_switch_aligner', 'air_flow_path', 'air_flow_aligner',
    'vfd_screen_aligner', 'vfd_pill_aligner', 'vfd_aligner', 'antifreeze_coil_valve', 'antifreeze_coil',
...
const ID_TO_TYPE = {
    ...
    'vfd_screen': 'vfd_screen_aligner', 'vfd_pill': 'vfd_pill_aligner', 'vfd_set': 'vfd_aligner',
    ...
    ['ahu_types', 'vav_types'].forEach(cat => {
        if(sanitized[cat]) {
            Object.keys(sanitized[cat]).forEach(id => {
                const anims = sanitized[cat][id]?.visual_assets?.animations || [];
                anims.forEach(a => { 
                ...
                const pts = sanitized[cat][id]?.points || [];
                pts.forEach(p => { 
