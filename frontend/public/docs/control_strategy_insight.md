...
## 5b. Dynamic Reset (ASHRAE Guideline 36) — the real-world competitor

Naked fixed-SA control is a strawman.  The actual modern competitor to
B1-B10 is **Dynamic Reset / Trim & Respond**, codified in ASHRAE
Guideline 36.  Instead of a single static SA setpoint, the AHU
re-targets its SA every few minutes based on what the served zones are
actually demanding:
...
- SA static pressure (fan-kW savings)
- Chilled water supply temperature (chiller COP)
- Hot water supply temperature (boiler condensing economy)

### How Dynamic Reset compares to B1-B10

| Aspect                       | Dynamic Reset (G36)                | B1-B10                         |
|------------------------------|-------------------------------------|---------------------------------|
| Foresight needed             | No (reacts to zone demand)          | No (reacts to OA condition)    |
| Multi-zone aware             | ✅ Yes (the whole point)            | ⚠️ Partial (VAV reheat handles) |
| Latent control               | Add-on (DOAS, dewpoint reset)       | ✅ Built-in (B7-B10 = dehumid)  |
| Implementation complexity    | High (zone request infrastructure)  | Low (flat lookup table)        |
| Hardware needed              | Per-zone controllers + BMS network  | Just OA T/RH sensor            |
| Failure mode                 | Stale request → drift               | OA fail → fallback to B5/B6    |
| Energy savings vs fixed-SA   | 30–55 % (ASHRAE figures)            | 60–80 % (chart data)           |
...
- **Buildings with highly variable internal loads** (offices with
  ...
  things OA-based logic can't see.
- **Tightly tuned existing BMS systems**: G36 sequences integrate well
  with major BMS platforms (Tridium, Schneider EBO, Siemens Desigo).
- **Spec-driven new construction**: ASHRAE 36 is the de facto US/AU
  standard.  LEED and NABERS often expect it.

### Where B1-B10 wins

- **Climate-driven envelope-dominated buildings** (hospitals, museums,
  schools): the dominant load is OA enthalpy.  OA-anchored bands track
  it more directly than zone-demand summing.
- **Edge controllers without BMS integration**: B1-B10 runs from a
  single OA sensor.  No zone-demand network required.
- **Latent-heavy climates** (Singapore, Manila, Taipei, Houston): the
  band table explicitly hands off to dehumidification SA targets
  (95 % RH, 11–13 °C) at the right thresholds.  G36 needs a separate
  dewpoint-reset rule on top.
- **Resilience**: a single misbehaving zone controller can drag a G36
  system into wasteful trimming.  B1-B10 is immune — its input is the
  weather, not the zones.

### How the chart estimates Dynamic Reset

The `Dyn-Rst` solid purple line uses a defensible textbook model:

> **SA enthalpy at hour `i`** = trailing-24-h mean of `h_oa`

This captures G36's aggregate behaviour: SA tracks recent OA conditions
on a slow time constant.  The cumulative becomes
`Σ |h_oa[i] − rolling_mean[i]|` — formally, an exponentially-decaying
centered absolute deviation.  The Dyn-Rst line typically sits between
the dashed Total (worst-case fixed-SA) and the dotted Opt-SA
(theoretical floor), at roughly 40–60 % of the Total magnitude — which
matches the published ASHRAE G36 savings range.
...
- B1-B10 ensures you're **never operating in the wrong regime** — no
  cooling when you should be dehumidifying, no 100 % OA when h_oa > h_sa
...
- The combined system is **failsafe**: zone telemetry loss → falls back
  to pure B1-B10 instead of stuck-at-default fixed-SA

This hybrid approach is the recommended commissioning posture for new
hospital projects using a Red5 Studio controller alongside an existing
G36-compliant BMS.
