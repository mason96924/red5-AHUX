 ...
 *   * VAV Terminal Hub lives on the AHU graphic modal (ahu-modal.js
 *     AhuVavHubSlideTab) — not docked on this chart when an AHU
 *     badge is selected in the sidebar.
...
<div className="flex-1 relative flex items-center justify-center overflow-hidden font-black shadow-black shadow-inner">
    ...
    {/* VAV Terminal Hub is a slide tab on the AHU graphic modal
        (AhuVavHubSlideTab in ahu-modal.js). Selecting an AHU badge in
        the sidebar no longer docks the table on this chart. */}
    ...
    <svg ref={svgRef} viewBox={`0 0 ${width} ${height}`} className={`w-full h-full cursor-crosshair transition-all duration-500 shadow-black shadow-black`}>
        ...
        {renderGrid()}{renderGivoniOverlay()}{typeof renderMoldOverlay === 'function' && renderMoldOverlay()}
