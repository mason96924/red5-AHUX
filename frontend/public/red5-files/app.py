...
def get_weather_location():
    try:
        if os.path.isfile(WEATHER_LOC_PATH):
...
def set_weather_location():
    try:
        ...
        # Minimal validation: require numeric lat/lon
        ...
        try:
            ...
            pass
        with open(WEATHER_LOC_PATH, 'w') as f:
...
def serve_js(filename):
    return send_from_directory('/root/data/js', filename)
...
@app.route('/')
def serve_landing():
    return send_from_directory('/root/data', 'landing.html')

@app.route('/dashboard')
def serve_dashboard():
    return send_from_directory('/root/data', 'dashboard.html')

@app.route('/mapper')
def serve_mapper():
    return send_from_directory('/root/data', 'equipment_mapper.html')
...
def weather_history():
    ...
    # Check cache first
    cache_file = os.path.join('/root/data', 'configs', f'weather_{lat}_{lon}_{year}.json')
    ...
    try:
        ...
        # Cache it (skip caching current year since data grows daily)
        import datetime
...
def get_tomorrow_forecast(lat, lon):
    ...
    # Try cache first
    cache_file = os.path.join('/root/data', 'configs', f'weather_{lat}_{lon}_{past_year}.json')
    data = None
    if os.path.isfile(cache_file):
        try:
            with open(cache_file, 'r') as f:
...
def download_bundle():
    """Generate a zip of ALL files in /root/data/ and /root/scripts/, encrypt with password, return as .red5 file."""
    try:
        data = request.json or {}
        password = data.get('password', '')
        if not password:
        ...
        return Response(
            ...
            headers={'Content-Disposition': 'attachment; filename=red5_bundle.red5'}
        )
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def serve_update_page():
    return send_from_directory('/root/data', 'update.html')
