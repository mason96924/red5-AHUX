...
_service_dependencies = [
    'DATA_ROOT', 'SCRIPTS_ROOT', 'ALLOWED_EXTENSIONS', 'MASTER_KEY_CONST',
...
def _finalize_bundle_from_disk(spool_path, password):
    ...
    try:
        ...
        if not ok:
            ...
            ok2, free_bytes, free_inodes = _check_free_space(DATA_ROOT, 20 * 1024 * 1024, 400)
            if not ok2:
                return {
                    'success': False,
    ...
    try:
        ...
        if chunk_index == 0:
            try:
                ...
                pass
            need = max(20 * 1024 * 1024, total_size * 3)
...
def register(app, ctx):
    ...
    ``ctx`` keys (all required):
        ...
        _derive_key, _no_cache
    """
    global DATA_ROOT, SCRIPTS_ROOT, ALLOWED_EXTENSIONS, MASTER_KEY_CONST
