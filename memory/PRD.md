...
- [x] **Redefined Opt-SA as a true thermodynamic floor** (P1) in BOTH chart paths:
  ...
  - T×Time (`renderTimeSeries2D`, `mode==='tt'`): same redefinition. The previous L2-optimal anchor produced the user-reported bug where Dyn-Rst (489 kJ/kg cumulative) was beating Opt-SA (824 kJ/kg) — physically impossible for a "floor". After fix, Opt-SA is the lowest visible curve (verified Adelaide preset, ~80 kJ/kg vs 13.2 k Dyn-Rst).
