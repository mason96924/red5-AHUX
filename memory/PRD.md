...
- [x] **Redefined Opt-SA as a true thermodynamic floor** (P1): replaced the L2-optimal `mean(h_oa)` target (no physical interpretation) with `h_sa_opt(t) = clamp(h_oa(t), _optMinH, _optMaxH)` where the comfort envelope defaults to `[25, 50] kJ/kg` and is user-configurable (Option C). When OA itself sits inside the envelope, conditioning load drops to zero — making Opt-SA the true unachievable lower bound for any setpoint-reset strategy.
